...
let gMapKey = "AIzaSyCQ5fPFIO7EOzgSCNVtlhQOV0_JcS3u8hM";
...
function displayBeer(beer) {
  //display beer from
  let defaultImage =
    beer.image_url === null ? $(".default-beer").attr("src") : beer.image_url;
  ...
  $(".go-beer-list").on("click", function () {
    ...
    } else $("#beer-list-container").removeClass("hidden");
  });
}
...
function filterList(list, input) {
  ...
  let newList = [];
  for (let k in list) {
    if (regex.test(list[k].name)) newList.push(list[k]);
  }
  return newList;
}
...
function displayVenue(venue) {
  ...
  $(".go-brewery-list").on("click", "button", function () {
    ...
    } else $("#brewery-list-container").removeClass("hidden");
  });

  $(".brewery-content").on("click", ".map", function () {
    window.open(
      `http://maps.google.com/maps?q="${directions}`,
      "noopener",
      "noreferrer"
    );
  });
...
function getVenueInfo(id) {
  //get venue info
  fetch(
    `https://api.untappd.com/v4/venue/info/${id}?&compact=true&client_id=${untappd}&client_secret=${secret}`
  )
...
function getVenue(name) {
  ...
  fetch(
    `https://api.untappd.com/v4/search/venue?q=${name.title}&client_id=${untappd}&client_secret=${secret}`
  )
    .then((response) => response.json())
    .then((responseJson) => {
...
function displayBrewery(brewery) {
  ...
  let latLng =
    brewery.location.brewery_lat + "," + brewery.location.brewery_lng;
  let address = {
...
function getBreweryInfo(brewery) {
  ...
  fetch(
    `https://api.untappd.com/v4/brewery/info/${brewery.brewery.brewery_id}?compact=true&client_id=${untappd}&client_secret=${secret}`
  )
    .then((response) => response.json())
    .then((responseJson) => {
...
function getBreweryID(name) {
  ...
  fetch(
    `https://api.untappd.com/v4/search/brewery?q=${name.title}&client_id=${untappd}&client_secret=${secret}`
  )
    .then((response) => response.json())
    .then((responseJson) => {
      let specBrew;
      for (let i = 0; i < responseJson.response.brewery.items.length; i++) {
        ...
        if (
          Math.round(item.lat) === Math.round(loc[0]) &&
          Math.round(item.lng) === Math.round(loc[1])
        ) {
...
function manageList(list) {
  ...
  $(".brewery-list").on("click", "a", function () {
    ...
    for (var n in list) {
      if (list[n].title === selection) getBreweryID(list[n]);
    }
    $("#brewery").removeClass("hidden");
  });
...
function getBreweryList(latLng) {
  ...
  $(".form-content").val("");
  fetch(
    `https://places.api.here.com/places/v1/discover/search?app_id=${appId}&app_code=${appCode}&q=brewery&at=${latLng}`
  )
    .then((response) => response.json())
...
function getLatLng(city) {
  ...
  fetch(
    `https://maps.googleapis.com/maps/api/geocode/json?address=${city}&key=${gMapKey}`
  )
    .then((response) => response.json())
    .then((responseJson) => {
      if (
...
function brewerySearch() {
  ...
  $("#brewery-criteria").on("submit", function (e) {
    e.preventDefault();
    $("#search-brewery").addClass("hidden");
    let city =
      $(".city-input").val().trim() + ",+" + $(".state-input").val().trim();
    getLatLng(city);
  });
  $("html").css("height", "100%");
}
